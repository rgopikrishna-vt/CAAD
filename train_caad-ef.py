...
from utils import weights_init, augment_bt_salt, getDiscriminatorEmbeddings, compute_cosinescore_dynamic, getEmdCentroids, L_supclass, gradient_penalty, augment_hil_anomaly
import pdb
import imageio
...
hildatabenign = np.load(resourcepath+'/'+'benign_hil.npy')
hildataanomaly = np.load(resourcepath+'/'+'anomaly_hil.npy')

netG = Generator().to(device)
netG.load_state_dict(torch.load(resourcepath + '/' + 'g.pth'))
netD = Discriminator().to(device)
netD.load_state_dict(torch.load(resourcepath + '/' + 'd.pth'))
...
for epoch in range(res_epoch, num_epochs):
    ...
    for i, data in enumerate(dataloader_train):
        ...
        anoms_hil = augment_hil_anomaly(hildataanomaly, bsize)
        benign_hil = augment_hil_anomaly(hildatabenign, bsize)
        ...
        for idx, _ in enumerate(range(critic_iterations)):
            ...
            critic_anomhil, proj_anomhil = netD(
                anoms_hil.to(device).float(), penul=True)
            critic_benhil, proj_benhil = netD(
                benign_hil.to(device).float(), penul=True)
            ...
            proj_anom1 = proj_anom1.reshape(bsize, -1)
            proj_anomhil = proj_anomhil.reshape(bsize, -1)
            proj_benhil = proj_benhil.reshape(bsize, -1)

            L_cont_neg = L_supclass(proj_real, proj_anom1)
            L_cont_pos = L_supclass(proj_anom1, proj_real)
            L_cont_hilanom = L_supclass(proj_real, proj_anomhil)
            L_cont_hilben1 = L_supclass(proj_anom1, proj_benhil)
            L_cont_hilben2 = L_supclass(proj_anomhil, proj_benhil)
            ...
            lossD = L_disc + L_cont_neg + L_cont_pos + \
                L_cont_hilanom + L_cont_hilben1 + L_cont_hilben2
